...
class tokenizer {
public:
    tokenizer(const string_view& in_cmd, string_list& tokens) : cmdline_(in_cmd), char_pos_(0), tokens_(tokens) {}
    bool at_end() const { return char_pos_ >= cmdline_.length(); }
    bool done() const { return at_end() || has_error(); }
    bool has_error() const;
    void advance() { char_pos_++; }
    void next() { while (!at_end() && is_white_space(cur())) advance(); }
    char cur(int step = 0) const { return cmdline_[char_pos_ + step]; }
    bool can_advance(int step = 1) const { return (char_pos_ + step) >= 0 && (char_pos_ + step) < cmdline_.length(); }
...
args_parser& args_parser::add_single_required_arg(string const& name_or_pattern,
  ...
  args_.insert({key,
                {name_or_pattern, string_list(), help,
                 arg_option(arg_type_string, arg_action_store_const, arg_flag_required)}});
  return *this;
}
...
bool args_parser::do_parse() {
  ...
  while (!at_end()) {
    ...
    if (arg && !key.empty()) {
      ...
      case arg_action_store_true:
        args_values_[key] = arg_value::new_bool(true);
        break;
      case arg_action_store_false:
        args_values_[key] = arg_value::new_bool(false);
        break;
