...
std::ext::string_piece FindExtension(const std::string* path) {
    size_t extension_offset = FindExtensionOffset(*path);
    if (extension_offset == std::string::npos)
        return std::ext::string_piece();
    return std::ext::string_piece(&path->data()[extension_offset],
...
const char* GetSourceType(const std::ext::string_piece& ext) {
  for (size_t i = 0; i < sizeof(kSourceTypeForExt)/sizeof(kSourceTypeForExt[0]); ++i) {
    if (std::ext::string_piece(kSourceTypeForExt[i].ext) == ext)
...
bool HasExplicitFileType(const std::ext::string_piece& ext) {
  return ext == "dart";
}

bool IsSourceFileForIndexing(const std::ext::string_piece& ext) {
...
void PBXFileReference::Print(std::ostream& out, unsigned indent) const {
  ...
  if (!type_.empty()) {
    ...
    std::ext::string_piece ext = FindExtension(&name_);
...
PBXFileReference* PBXGroup::AddSourceFile(const std::string& navigator_path,
  ...
  std::ext::string_piece component(navigator_path.data(), sep);
  for (const auto& child : children_) {
    if (child->Class() != PBXGroupClass)
      continue;

    PBXGroup* child_as_group = static_cast<PBXGroup*>(child.get());
    if (child_as_group->name_ == component.to_string()) {
  ...
  if (!group) {
    children_.push_back(std::make_unique<PBXGroup>(component.to_string(),
                                                   component.to_string()));
...
void PBXProject::AddSourceFile(const std::string& navigator_path,
  ...
  std::ext::string_piece ext = FindExtension(&source_path);
...
PBXNativeTarget* PBXProject::AddNativeTarget(
  ...
  std::ext::string_piece ext = FindExtension(&output_name);
